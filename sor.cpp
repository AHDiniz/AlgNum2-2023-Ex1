...
DEFUN_DLD(sor, args, nargout, "This function implements the iterative Successive Over-Relaxation method for Ax = b linear systems.")
{
    Matrix A           = args(0).matrix_value();
    ColumnVector b     = args(1).column_vector_value();
    const double tol   = args(2).double_value();
    const int max_iter = args(3).int_value();
    const double omega = args(4).double_value();
    ...
    while (er(iter) > tol && iter < max_iter)
    {
        for (int i = 0; i < n; ++i)
        {
            ...
            for (int j = i + 1; j < n; ++j)
                sum += A(i,j) * x0(j);
            x(i) = omega * (b(i) - sum) / A(i,i) + (1 - omega) * x0(i);
        }
